...
Liste todas as colunas necessárias (movie_title, imdb_rating e year_released) na declaração SELECT, separadas por vírgula.
...
escreva a coluna box_office e use o operador "maior que" (>) para mostrar somente os valores acima de US$ 300 milhões.*/
...
Para localizar a palavra em qualquer lugar do título do filme, coloque o caractere curinga (%) antes e depois da palavra.*/
...
Use a mesma lógica da primeira condição, desta vez usando o operador "maior que" com a coluna imdb_rating*/

/*Exercício 6: Filtragem de dados usando WHERE e classificando o resultado
Exercício: Selecione as colunas movie_title, imdb_rating e year_released da tabela movies. Mostre os filmes lançados depois de 1991.
...
/*Explicação da solução: Liste as colunas em SELECT e faça referência à tabela em FROM.
Filtre os dados com WHERE aplicando o operador "maior que" à coluna year_released.
...
/*Explicação da solução: Selecione a coluna language na tabela movies.
Para calcular o orçamento médio, use a função de agregação AVG() com a coluna budget entre parênteses.
...
Nela, use a mesma construção AVG() que em SELECT e defina os valores como sendo acima de 50 usando o operador "greater than".*/
...
Na cláusula FROM, faça referência à tabela distribution_companies.
